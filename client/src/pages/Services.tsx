      ...
      <div className="pt-24">
        ...
        <section className="mx-auto max-w-[1500px] px-6 py-16 md:px-12 lg:px-24 lg:py-24">
          <div className="mb-10 flex items-end justify-between gap-6 border-b border-black/10 pb-5">
            ...
            <span className="t-data text-xs text-black/60" dir="ltr">{String(SERVICE_PACKAGES.length).padStart(2, "0")} / 05</span>
          </div>
            ...
            {SERVICE_PACKAGES.map((service, index) => {
              ...
              return (
                <motion.article
                  ...
                  className={`bg-[#fdfcfb] p-6 md:p-8 ${index === 0 ? "md:col-span-2 xl:col-span-2" : ""}`}
                >
                  <div className="mb-8 flex items-start justify-between gap-5">
                    ...
                    <span className="t-data text-xs text-black/60" dir="ltr">SVC_{String(index + 1).padStart(2, "0")}</span>
                  </div>
                  <h3 className="max-w-lg text-2xl font-bold tracking-tight">{service.name[lang]}</h3>
                  <p className="mt-4 max-w-xl text-sm leading-relaxed text-black/70">{service.scope[lang]}</p>
                  <dl className="mt-8 space-y-4 border-t border-black/10 pt-5 text-sm">
                    <div><dt className="t-meta text-black/60">{t.services.deliverableLabel}</dt><dd className="mt-1 font-semibold">{service.deliverable[lang]}</dd></div>
                    <div><dt className="t-meta text-black/60">{t.services.bestForLabel}</dt><dd className="mt-1 text-black/75">{service.bestFor[lang]}</dd></div>
                    <div><dt className="t-meta text-[#5a1f2e]">{t.services.limitationLabel}</dt><dd className="mt-1 text-black/75">{service.limitation[lang]}</dd></div>
                  </dl>
                  <div className="mt-7 border-t border-dashed border-black/10 pt-4 text-xs leading-relaxed text-black/60">
                    <span className="font-semibold text-black/75">{t.services.basisLabel}: </span>{service.basis[lang]}
                  </div>
                  <Link href={localizedLinkPath(`/match?service=${service.id}`, lang)} className="mt-7 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-[#5a1f2e] transition-[transform,color] hover:translate-x-1 rtl:hover:-translate-x-1 active:scale-[0.98]">
                    {t.services.inquiryCta} <ArrowRight size={14} strokeWidth={1.5} className="rtl:-scale-x-100" />
                  </Link>
                </motion.article>
              );
            })}
        ...
        <section className="border-y border-black/10 bg-white">
          <div className="mx-auto flex max-w-[1500px] flex-col gap-6 px-6 py-10 md:flex-row md:items-center md:justify-between md:px-12 lg:px-24">
            <p className="max-w-3xl text-sm leading-relaxed text-black/70">{t.services.footerNote}</p>
            <div className="flex flex-wrap gap-3">
              <Link href={localizedLinkPath("/match", lang)} className="inline-flex items-center gap-2 bg-[#5a1f2e] px-5 py-3 text-xs font-bold uppercase tracking-wider text-white transition-colors hover:bg-black active:scale-[0.98]">{t.services.inquiryCta} <ArrowRight size={14} strokeWidth={1.5} className="rtl:-scale-x-100" /></Link>
              <Link href={localizedLinkPath("/projects", lang)} className="inline-flex items-center border border-black/15 px-5 py-3 text-xs font-bold uppercase tracking-wider text-[#0b0b10] transition-colors hover:border-[#5a1f2e] hover:text-[#5a1f2e] active:scale-[0.98]">{t.services.portfolioCta}</Link>
            </div>
          </div>
        </section>
      </div>
