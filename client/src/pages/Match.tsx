        ...
        <section className="mx-auto grid max-w-[1500px] grid-cols-1 gap-10 px-6 py-16 md:px-12 lg:grid-cols-12 lg:px-24 lg:py-24">
          ...
          <aside className="lg:col-span-5">
            <div className="sticky top-28 border border-black/10 bg-[#0b0b10] p-6 text-white md:p-8">
              ...
              <p className="mt-4 text-sm leading-relaxed text-white/80">{t.match.previewNote}</p>
              {rankedProjects.length === 0 ? <p className="mt-8 border-t border-white/10 pt-6 text-sm leading-relaxed text-white/70">{form.sectors.length === 0 && form.countries.length === 0 && !form.targetProject ? t.match.previewPrompt : t.match.noMatch}</p> : <div className="mt-8 space-y-4">{rankedProjects.map(({ project, score, reasons }) => <motion.div key={project.slug} initial={false} animate={{ opacity: 1, x: 0 }} transition={{ duration: shouldReduceMotion ? 0 : 0.3 }} className="border-t border-white/10 pt-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-base font-semibold">{project.title[lang]}</h3><p className="mt-1 text-xs text-white/60">{COUNTRIES[project.country][lang]} · {SECTORS[project.sector][lang]}</p></div><span className="t-data shrink-0 text-xs text-[#f2a007]" dir="ltr">{t.match.scoreLabel}: {score}</span></div><ul className="mt-3 space-y-1 text-xs text-white/75">{reasons.map((reason) => <li key={reason}>+ {reason}</li>)}</ul><p className="mt-3 text-[11px] uppercase tracking-wider text-white/60">{STATUSES[project.status][lang]}</p><Link href={localizedLinkPath(`/projects/${project.slug}`, lang)} className="mt-4 inline-block text-xs font-bold uppercase tracking-wider text-[#f2a007] hover:text-white active:scale-[0.98]">{t.services.portfolioCta}</Link></motion.div>)}</div>}
              <p className="mt-8 border-t border-white/10 pt-5 text-xs leading-relaxed text-white/60">{t.match.limitation}</p>
            </div>
          </aside>
        </section>
      </div>
