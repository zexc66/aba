        ...
        <Section className="py-16 border-b border-black/10 bg-white">
          <div className="mx-auto max-w-[1500px] px-6 md:px-12 lg:px-24">
            ...
            >
              ...
              <p className="mt-8 max-w-[65ch] text-base md:text-lg text-black/70 leading-relaxed">
                {t.heroNote}
              </p>
            </motion.div>
          </div>
              ...
              {t.pillars.map((pillar: { title: string; desc: string; code: string }, i: number) => (
                ...
                >
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <span className="t-meta text-black/60">{pillar.code}</span>
                  </div>
                  <h3 className="text-xl font-bold text-[#0b0b10] mb-3">{pillar.title}</h3>
                  <p className="text-sm text-black/70 leading-relaxed">{pillar.desc}</p>
                </motion.div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              >
                ...
                <Link asChild href={localizedLinkPath("/pipeline", lang)}>
                  <a className="inline-flex items-center gap-2 t-meta text-[#5a1f2e] mt-6 border-b border-[#5a1f2e]/40 hover:border-[#5a1f2e] pb-1 pt-2 transition-colors">
                    SDG_ALIGNMENT_MATRIX <ArrowRight size={12} strokeWidth={1.5} className="rtl:-scale-x-100" />
                  </a>
                </Link>
              </motion.div>
              ...
              >
                ...
                <p className="text-sm md:text-base text-white/80 leading-relaxed">{t.agendaFrame}</p>
              ...
              {t.horizons.map((h: { sector: string; vision: string }, i: number) => (
                ...
                >
                  <div className="flex items-baseline gap-3">
                    <span className="t-data text-xs text-black/60" dir="ltr">
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <h3 className="text-base font-bold text-[#5a1f2e]">{h.sector}</h3>
                  </div>
                  <p className="text-sm md:text-base text-black/70 leading-relaxed max-w-[65ch]">
                    {h.vision}
                  </p>
                </motion.li>
              ))}
          ...
          <div className="mx-auto max-w-[1500px] px-6 md:px-12 lg:px-24">
              ...
              {content.team.list.map((member, i) => (
                ...
                >
                  ...
                  <Link asChild href={localizedLinkPath(`/team/${["mohammed-abdel-moneim", "faris-safi", "ziad-shneikat"][i] ?? ""}`, lang)}>
                    <a className="inline-flex items-center gap-2 t-meta text-black/60 hover:text-[#5a1f2e] mt-4 transition-colors py-2">
                      PROFILE <ArrowRight size={12} strokeWidth={1.5} className="rtl:-scale-x-100" />
                    </a>
                  </Link>
                </motion.div>
              ))}
            ...
            <div className="mt-14 flex justify-center">
              <a
                ...
                className="inline-flex items-center gap-3 bg-[#5a1f2e] hover:bg-[#0b0b10] text-white px-8 py-4 font-semibold text-sm transition-colors"
              >
