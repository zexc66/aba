...
const COPY = {
  en: { title: "Inquiry workflow", stage: "Stage", assignee: "Assigned owner", nextAction: "Next action", save: "Save workflow", history: "Workflow history", failed: "Update failed. Reload to check for another operator’s changes before retrying.", new: "New", qualified: "Qualified", introduced: "Introduction arranged", assessment: "Assessment", "active-development": "Active development", paused: "Paused", closed: "Closed", saving: "Saving…", done: "Workflow saved", createRoom: "Create project room", roomBusy: "Creating…", roomReady: "Project room ready", roomLinked: "Linked to existing room", roomInvited: "Invitation email sent to the lead", roomFailed: "Room creation failed. Retry or check server logs.", openRoom: "Open room" },
  ar: { title: "سير عمل الاستفسار", stage: "المرحلة", assignee: "المسؤول المعين", nextAction: "الإجراء التالي", save: "حفظ سير العمل", history: "سجل التحديثات", failed: "فشل التحديث. أعد التحميل للتحقق من تعديلات مسؤول آخر قبل المحاولة.", new: "جديد", qualified: "مؤهل", introduced: "تم ترتيب التعريف", assessment: "التقييم", "active-development": "تطوير نشط", paused: "متوقف مؤقتاً", closed: "مغلق", saving: "جارٍ الحفظ…", done: "تم حفظ سير العمل", createRoom: "إنشاء غرفة مشروع", roomBusy: "جارٍ الإنشاء…", roomReady: "غرفة المشروع جاهزة", roomLinked: "مرتبط بغرفة قائمة", roomInvited: "أُرسل بريد الدعوة إلى صاحب الاستفسار", roomFailed: "فشل إنشاء الغرفة. أعد المحاولة أو راجع سجلات الخادم.", openRoom: "فتح الغرفة" },
  fr: { title: "Suivi de la demande", stage: "Étape", assignee: "Responsable assigné", nextAction: "Prochaine action", save: "Enregistrer le suivi", history: "Historique du suivi", failed: "Échec de la mise à jour. Rechargez pour vérifier les modifications d’un autre opérateur avant de réessayer.", new: "Nouvelle", qualified: "Qualifiée", introduced: "Mise en relation organisée", assessment: "Évaluation", "active-development": "Développement actif", paused: "En pause", closed: "Clôturée", saving: "Enregistrement…", done: "Suivi enregistré", createRoom: "Créer un salon de projet", roomBusy: "Création…", roomReady: "Salon de projet prêt", roomLinked: "Lié à un salon existant", roomInvited: "E-mail d’invitation envoyé au contact", roomFailed: "Échec de la création du salon. Réessayez ou consultez les journaux du serveur.", openRoom: "Ouvrir le salon" },
};
...
export default function LeadWorkflow({ lead, token, locale, onUpdated }: { lead: WorkflowLead; token: string; locale: Locale3; onUpdated: () => void }) {
  ...
  const [room, setRoom] = useState<{ id: string; created: boolean; invited: boolean } | null>(null);
  const [roomState, setRoomState] = useState<"idle" | "busy" | "failed">("idle");
  ...
  async function createRoom() {
    ...
    setRoomState("busy");
    try {
      const response = await fetch(`/api/admin/leads/${lead.id}/room`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-admin-token": token },
        body: JSON.stringify({ revision: lead.revision ?? 0 }),
      });
      if (!response.ok) throw new Error("Room creation rejected");
      const body = (await response.json()) as { roomId: string; created: boolean; invited: boolean };
      setRoom({ id: body.roomId, created: body.created, invited: body.invited });
      onUpdated();
    } catch {
      setRoomState("failed");
      return;
  ...
  return <details className="border-t border-white/20 pt-3" dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    <form onSubmit={save} className="space-y-4 pb-5">
      ...
      <div className="border-t border-white/10 pt-4">
        <button type="button" onClick={() => void createRoom()} disabled={roomState === "busy"} className="min-h-11 border border-white/30 px-4 py-2 text-sm text-[#fdfcfb] hover:border-[#f2a007] hover:text-[#f2a007] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#f2a007] disabled:opacity-50">
          {roomState === "busy" ? t.roomBusy : t.createRoom}
        </button>
        {room && (
          <p role="status" className="mt-3 text-sm">
            {room.created ? t.roomReady : t.roomLinked}
            {room.invited ? ` · ${t.roomInvited}` : ""}{" "}
            <a href={`/rooms/${room.id}`} className="text-[#f2a007] underline underline-offset-4" target="_blank" rel="noreferrer">{t.openRoom}</a>
          </p>
        )}
        {roomState === "failed" && <p role="alert" className="mt-3 text-sm">{t.roomFailed}</p>}
      </div>
    </form>
