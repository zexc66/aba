...
export default function Companies() {
  ...
  return <NetworkLayout title={t.directory} description={t.intro} path="/companies">
    ...
    <ul className="divide-y divide-[#0b0b10]/10 border-y border-[#0b0b10]/10">
      {matches.map(company => <li key={company.slug} className="grid min-w-0 gap-5 py-7 sm:grid-cols-[7rem_1fr] lg:grid-cols-[7rem_1fr_auto] lg:items-center">
        <img src={deployAssetPath(company.logo)} alt="" loading="lazy" decoding="async" className="h-20 w-28 object-contain" />
        ...
        <Link asChild href={localizedLinkPath(`/companies/${company.slug}`, lang)}><a className={networkLink}>{t.profile}<ArrowRight size={16} className="rtl:-scale-x-100" aria-hidden="true" /></a></Link>
      </li>)}
    </ul>
