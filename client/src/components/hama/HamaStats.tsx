...
export default function HamaStats({ stats }: HamaStatsProps) {
    ...
    return (
        <section className="relative py-72 md:py-80 px-6 sm:px-8 lg:px-24 bg-[#fdfcfb] border-y border-black/[0.08] overflow-x-clip">
            <motion.div
                initial={initial}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.18 }}
                transition={{ duration: 0.75, ease: [0.32, 0.72, 0, 1] }}
                className="bg-black/[0.04] border border-black/[0.08] p-1.5 shadow-premium-2xl"
            >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 grid-flow-dense gap-px bg-black/[0.08] border border-black/[0.05] shadow-[inset_0_1px_1px_rgba(253,252,251,0.9)]">
                    {stats.map((stat, i) => (
                        <motion.div
                            key={stat.id}
                            initial={initial}
                        ...
                        >
                            ...
                            <div className="flex items-start justify-between gap-8 relative z-10 font-sans">
                                <div className="flex flex-col gap-3 min-w-0">
                                    <span className="t-data text-[10px] font-black tracking-[0.45em] uppercase text-black/25 group-hover:text-[#5a1f2e] transition-colors duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] tabular-nums" dir="ltr">
                                        {stat.id}
                                    </span>
                                    <div className="w-12 h-px bg-black/[0.08] group-hover:bg-[#5a1f2e]/40 transition-colors duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]" />
                                </div>
                                <div className="w-14 h-14 sm:w-16 sm:h-16 border border-black/[0.07] flex items-center justify-center text-black/25 group-hover:text-[#5a1f2e] group-hover:border-[#5a1f2e]/35 transition-[color,border-color,transform] duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] bg-[#fdfcfb] shadow-[0_18px_40px_-24px_rgba(90,31,46,0.45)] group-hover:scale-[1.02]">
                            ...
                            <div className="relative z-10">
                                ...
                                <h3 className="t-meta text-[12px] sm:text-[13px] font-black tracking-[0.42em] uppercase text-black/45 group-hover:text-[#0b0b10] transition-colors duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] max-w-[16rem]">
                                    {stat.label}
                                </h3>
                            </div>
