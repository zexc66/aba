...
class CMSService {
    ...
    private initClient() {
        ...
        if (space && accessToken) {
            ...
            if (import.meta.env.DEV) {
                console.warn("CMS: Contentful keys not detected in environment. Falling back to local/mock data.");
            }
        }
    }
    ...
    async getWebsiteContent(locale: Locale = "en"): Promise<typeof COPY["en"]> {
        ...
        try {
            const contentfulLocale = locale === "en" ? "en-US" : locale === "ar" ? "ar-SA" : "fr-FR";
            ...
            if (response.items && response.items.length > 0) {
                ...
                console.error("CMS: 'siteSettings' entry is missing expected 'contentData' field. Using local data.");
            }

            throw new Error("Contentful entries empty or malformed");
        } catch (error) {
            console.error("Failed to fetch from Contentful. Falling back to static data.", error);
            return COPY[locale];
        }
    }
