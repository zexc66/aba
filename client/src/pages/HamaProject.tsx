...
import { School, Hospital, Users, Calendar } from "lucide-react";
...
const HAMA_CONTENT = {
  en: {
    ...
    cta: {
      ...
      subtitle:
        "Sustain the bridge between regional crisis and community sovereignty.",
  ...
  fr: {
    ...
    cta: {
      ...
      subtitle:
        "Soutenir le pont entre la crise régionale et la souveraineté communautaire.",
...
export default function HamaProject() {
  ...
  const t = useMemo(
    () => HAMA_CONTENT[lang as keyof typeof HAMA_CONTENT] || HAMA_CONTENT.en,
    [lang]
  );
  ...
  // MotionConfig reducedMotion only covers declarative animations, not scroll-linked
  // MotionValues — flatten the parallax ranges instead.
  const reduceMotion = useReducedMotion();
  const heroLetterY = useTransform(
    smoothProgress,
    [0, 0.2],
    reduceMotion ? [0, 0] : [0, -400]
  );
  ...
  return (
    <div
      ref={containerRef}
      data-hama
      className={`bg-[#fdfcfb] text-[#0b0b10] ${isRTL ? "font-arabic" : "font-institutional"} overflow-hidden`}
    >
      <SEO
      ...
      {/* ponytail: scoped to [data-hama] so these overrides stop leaking into every
                other route's .text-8xl / .hud-label while this page is mounted. */}
      <style
        dangerouslySetInnerHTML={{
          __html: `
                [data-hama] .text-8xl { font-size: clamp(4rem, 8vw, 15vw); }
                [data-hama] .tracking-tightest { letter-spacing: -0.08em; }
                [data-hama] .hud-label {
                    font-size: 8px;
                    font-weight: 900;
